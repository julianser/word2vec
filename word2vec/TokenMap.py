...
class TokenMap(object):
	...
	def compact(self):
		"""
		Recreates the TokenMap after removing all the None's which are
		created when remove is called
		"""
		self.tokens = [t for t in self.tokens if t is not None]
		self.map = dict((t, idx) for idx, t in enumerate(self.tokens))

	def remove(self, token):
		"""Removes token from the Map"""
	...
	def add_list(self, token_iterable):
		"""Adds a list of tokens"""
	...
	def get_ids(self, token_iterable):
		"""Gets IDs of all the tokens in the iterable"""
		return [self.get_id(token) for token in token_iterable]

	def get_token(self, idx):
		"""Gets token mapped to an ID"""
		return self.tokens[idx]

	def get_tokens(self, idx_iterable):
		"""Gets tokens mapped to a list of IDs"""
