"""
 This module contains DatasetReader class and all functions
 necessary for loading and parsing datasets
"""
import re
...
class DatasetReader(object):
	...
	def generate_filenames(self):
		...
		* [str]: absolute path to a dataset file
		...
		# Process the files listed in `files`, unless matches entry in skip
	...
	def generate_examples(self, filename_iterator):
		"""
		Using the data of a parsed file, generates examples.
		"""

		num_examples = 0
		selector = TokenSelector(K=len(self.kernel) / 2, kernel=self.kernel)
		# sorted_examples = []
		...
		for filename in filename_iterator:
			...
			for tokens in parsed:
				...
				for query_token_pos, query_token_id in enumerate(token_ids):
					...
					context_token_pos = selector.choose_token(
	...
	def generate_macrobatches(self, filename_iterator):
		"""
		Assembles batches of examples from the parsed data coming from
		...
		if len(example) > 0:
			...
			macrobatch = numpyify(
				example + [padding_row] * examples_remaining)
	...
	def generate_dataset_worker(self, file_iterator, macrobatch_queue):
		"""Generates Dataset Worker"""
		macrobatches = self.generate_macrobatches(file_iterator)
		for examples in macrobatches:
			if self.verbose:
				print 'sending macrobatch to parent process'
			macrobatch_queue.put(examples)
	...
	def generate_dataset_parallel(self, save_dir=None):
		...
		for macrobatch in macrobatch_consumer:
